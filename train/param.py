...
parser.add_argument("--log_dir", type=str , default="./tensorboard_logs/" , help="directory path for generate logging information")
parser.add_argument("--vocab_path", type=str , default="../vocab/" , help="directory path for vocabulary information")
...
'''
parser.add_argument("--grad_clip" , type=float , default=2.0 , help="threshold of clip gradient")
'''
...
parser.add_argument("--log_step" , type=int , default=10 , help="number of steps for logging") # 每训练 log_step 个batch就会记录并输出一次训练指标
parser.add_argument("--val_step" , type=int , default=500 , help="number of steps for validation") # 每训练 val_step 个batch就会使用验证集对模型进行评估
parser.add_argument("--lr_decay_step", type=int , default=15 , help='number of epochs to decay the learning rate')  # 每经过 lr_decay_step 个epoch就衰减一次学习率
...
parser.add_argument("--num_heads" , type=int , default=3 , help='number of heads of multi-head attention')
parser.add_argument("--use_word2vec" ,action='store_true' , default=False , help='whether to use Gensim to initialize word embedding layer')
parser.add_argument("--model_class" , type=str , default='ViT_and_BERT' , help='choose what kind of model to use (CNN_and_GRU , ViT_and_BERT , CNN_and_BERT)')
parser.add_argument("--max_violation_in_middle" ,action='store_true' , default=False, help='choose whether to use max_violation loss in the middle of training')
